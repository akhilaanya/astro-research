...
errpos_met = np.maximum(0, data[:,7])
errpos_met[errpos_met < 0] = 0
errneg_met = np.maximum(0, data[:,8])
errneg_met[errneg_met < 0] = 0

verterr = np.array([errneg_met, errpos_met])

plt.hlines(metallicity, np.minimum(LAS_gyr, LAE_gyr),
           np.maximum(LAS_gyr, LAE_gyr),
...
plt.title("ANDIV Metallicity/Age")
